...
} , 
{
  ...
  const infoTextShort = '"ליפט פיינדר" הוא כלי עזר ייחודי למציאת כלים מתאימים לעבודה בגובה. הוא מציע ערך מוסף למגוון המקצועי, עם אוסף של 35 כלים שונים כמו במות הרמה, במות מספריים, במות טלסקופיים ועוד. '
...
<View style={styles.itemCart}>
  ...
  <Text style={styles.titleItemCart}> כלי: {title}</Text>
  </View>
</View>
);
  ...
  const sendOrder = () => {
    ...
    navigation.navigate('OrderHandler',{orderData:cartData})
  }
     ...
     <Modal  //cart modal
      ...
      />
            ...
            <View style={styles.CartmodalInfoTailS}>
              <TouchableOpacity onPress={sendOrder} style={styles.cartModalSendOrder}>
              <Text style={styles.modalTextTail}>שלח הזמנה</Text>
              </TouchableOpacity>
            </View>
      ...
      <View style={styles.dataRange}>
        ...
        <Pressable onPress={filterImgs} style={styles.fliterData}>
          <Image source={searchIcon } style={styles.filterimg} />
          <Text disabled={false} style={styles.filterbtntext}>חפש</Text>         
    </Pressable>


    <Pressable  style={styles.resetdata} onPress={resetFilters}>
          <Image source={reset}  style={styles.resetimg} />
          <Text style={styles.infotext}>אפס</Text>
    </Pressable>
...
const styles = StyleSheet.create({
  ...
  infos:{
    ...Dimensions.get('window').width>815?{flex:3}:{flex:6},
  ...
  datafilter:{
    ...
    ...Platform.select({
      ...
      web:{
        ...Dimensions.get('window').width>815?{flex:9}:{flex:6},
